...
run_one_id <- function(id){
  ...
  #You really should look at how well stuff fits at its worst! This is what this step does: checks the worst fits are still okay
  best.old <- best.genes
 ...
 #   print(t)
  ...
  outputlist[["genes"]][['worst']] <- best.genes
  outputlist[["MSE"]][['worst']] <- best.model$mse
  
  
  i <- match(id,ids)
  time_per_sample <- (Sys.time()-t0)/i
  samples_remaining=length(ids)-i
#  print(time_per_sample*samples_remaining)
